...
$esFavorito = false;
if ($usuario_logueado) {
    // Usar la conexión global que ya tienes configurada
    global $db;
    if ($db) {
...
<div class="container my-5">
  <div class="row g-4">
    ...
    <div class="col-md-6">
      ...
      <?php if (!empty($producto->N_Talla)): ?>
        <p class="text-muted">Talla base: <?= htmlspecialchars($producto->N_Talla); ?></p>
      <?php endif; ?>
        ...
        <?php if ($usuario_logueado): ?>
          <form id="fav-form" method="post" action="<?= BASE_URL; ?>?c=Favorito&a=toggle" style="display:inline-block">
            ...
            <button type="submit" class="btn <?= ($esFavorito ? 'btn-danger' : 'btn-outline-danger'); ?>">
              <i class="fas fa-heart"></i> <?= ($esFavorito ? 'Quitar de Me Gusta' : 'Añadir a Me Gusta'); ?>
            </button>
          </form>
        <?php else: ?>
          <a href="<?= BASE_URL; ?>?c=Usuario&a=login" class="btn btn-outline-secondary">
            <i class="fas fa-heart"></i> Inicia sesión para favoritos
          </a>
        <?php endif; ?>
      </div>
      ...
      <div id="color-container" class="d-flex flex-wrap">
        ...
        $coloresMostrados = [];
        // Artículo base
        echo '<div class="chip active" data-id="' . htmlspecialchars($producto->ID_Color ?? 'base') . '" data-foto="' . htmlspecialchars($fotoUrl) . '" data-nombre="' . htmlspecialchars($producto->N_Articulo) . '" data-base="1">
        ...
        foreach ($variantesPorColor as $c) {
          ...
          echo '<div class="chip" data-id="' . $c['ID_Color'] . '" data-foto="' . htmlspecialchars($fotoVarUrl) . '" data-nombre="' . htmlspecialchars($producto->N_Articulo . ' - ' . $c['N_Color']) . '">
      ...
      <div id="talla-container" class="mb-3">
        <!-- Las tallas se cargarán dinámicamente por JavaScript -->
      ...
      <form id="add-cart-form" method="post" action="<?= BASE_URL; ?>?c=Carrito&a=agregar">
        ...
        <button id="btn-add-cart" type="button" class="btn btn-dark w-100 btn-cart-disabled" disabled>
          <i class="fas fa-shopping-cart"></i> Selecciona una talla
        </button>
      </form>
    </div>
  </div>
  ...
  <!-- Variantes -->
  <h4><i class="fas fa-palette"></i> Variantes disponibles</h4>
  <div class="row mt-3">
          ...
          <div class="card card-variant h-100 p-2 text-center shadow-sm">
            ...
            <?php foreach ($c['opciones'] as $o): ?>
              <div class="d-flex justify-content-between align-items-center mt-2 p-2 border rounded">
                <div>
                  <strong><?= htmlspecialchars($o['N_Talla']); ?></strong><br>
...
<!-- Si tienes un archivo ver.js específico -->
<?php if (file_exists('assets/js/ver.js')): ?>
